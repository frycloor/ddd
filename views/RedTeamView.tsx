...
import type { MapPanelRef } from '../components/redteam/MapPanel';
const MapPanel = React.lazy(() => import('../components/redteam/MapPanel'));
...
const RedTeamView: React.FC = () => {
  ...
  return (
    <div className="p-4 grid grid-cols-12 grid-rows-6 gap-4 h-full bg-gray-900">
      ...
      <div className="col-span-6 row-span-4 rounded-lg overflow-hidden bg-gray-800 border border-gray-700">
        <Suspense fallback={<div className="h-full w-full flex items-center justify-center text-sm text-gray-400">Loading map...</div>}>
          <MapPanel ref={mapPanelRef} onSimulationComplete={() => setIsSimulating(false)} />
        </Suspense>
      </div>
